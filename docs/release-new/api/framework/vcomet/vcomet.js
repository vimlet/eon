...
// prototype $ and $1 should not conflict with other framework API 
...
vcomet.insertImport = function (href) {
    ...
    var elementName = href.match(/[^\/]*$/g)[0].replace(".html", "").toLowerCase();
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (scripts.length > 0) {
                    ...
                    for (i = 0; i < scripts.length; i++) {
                        ...
                        if (scripts[i].src || scripts[i].getAttribute("data-src")) {
                            ...
                            vcomet.imports.script += scripts[i].innerHTML;
...
vcomet.handleScriptsAppend = function () {

    var elementNames = Object.keys(vcomet.imports.scripts);
    var combinedScript = document.createElement("script");
    ...
    for (var i = 0; i < elementNames.length; i++) {
        ...
        for (var j = 0; j < elementScriptsKeys.length; j++) {
            document.head.appendChild(elementScripts[elementScriptsKeys[j]]);
        }

    }

    // Append combined script with auto-remove
    vcomet.imports.combinedScript = document.createElement("script");
    vcomet.imports.combinedScript.innerHTML = vcomet.imports.script;
    vcomet.imports.combinedScript.innerHTML += "\n";
    vcomet.imports.combinedScript.innerHTML += "vcomet.imports.combinedScript.parentNode.removeChild(vcomet.imports.combinedScript);";
    document.head.appendChild(vcomet.imports.combinedScript);
...
vcomet.element = function (name, config) {
...
vcomet.importTemplateClasses = function (el) {
    ...
    if (template && template.classList && template.classList.length != 0) {
